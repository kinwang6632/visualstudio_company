...
Public Class BillingAPI256
    Inherits CableSoft.BLL.Utility.BLLBasic
    Implements IDisposable, CableSoft.BLL.BillingAPI.IBillingAPI
    Private _DAL As New BillingAPI256DAL(Me.LoginInfo.Provider)
    'Private _DAL As New BillingAPI256DALMultiDB(Me.LoginInfo.Provider)
    Private Lang As New CableSoft.BLL.Language.SO61.BillingAPI256Language
    '回應碼	回應狀態	回應訊息
    ' 0     成功
    '-1	    失敗	{參數}資料有誤!!
    '-304	失敗	客編不存在
    ...
    Public Function Execute(SeqNo As Integer, InData As System.Data.DataSet) As CableSoft.BLL.Utility.RIAResult Implements CableSoft.BLL.BillingAPI.IBillingAPI.Execute
        ...
        'SNO	單	    Faciseqno	設備流水號		V       一次一個設備
        'SNO	單	    CitemCode	收費項目		V       正項收費項目     ex: 100,101,102
        ...
        End Using
        Dim CitemCodeString As String = InData.Tables("SNO").Rows(0)("CitemCode")
        For Each citemCode As String In CitemCodeString.Split(",")
            '檢核是否可派工
            result = CheckCanPR(InData, ServiceType, WipRefNo, ReInstAcrossFlag, PRCode, citemCode)
            If result.ResultBoolean = False Then
                Return result
            End If
            result = WipDataSave(InData, ServiceType, WipRefNo, ReInstAcrossFlag, PRCode, citemCode)
    ...
    Private Function WipDataSave(InData As DataSet, ByVal ServiceType As String,
        ...
        Using objBll As New PR(Me.LoginInfo, DAO)
            Dim strChooseserviceid As String = objBll.setChooseServiceIdByCitem(
                                                        WipData.Tables("Wip").Rows(0).Item("CustId").ToString,
                                                                               CanChooseCitemCode, _
    ...
    Private Function CheckCanPR(InWipData As DataSet, ByRef ServiceType As String,
        ...
        Using Vali As New Validate(LoginInfo, DAO)
            ...
            If Not String.IsNullOrEmpty(InRow("ReasonDescCode").ToString) Then
                ...
                End Using
            End If
            '根據SO041參數取派工類別 By Kin 2019/11/21 For Debby
            Dim intStartNewProd As Integer = Integer.Parse(DAO.ExecSclr(_DAL.GetStartNewProd()).ToString)
            Using dtCD007 As DataTable = DAO.ExecQry(_DAL.GetWorkCodeForCitemCode(CitemCode, intStartNewProd))
                If dtCD007.Rows.Count > 0 Then
                    intPRCode = dtCD007.Rows(0)("CodeNo")
                    ServiceType = dtCD007.Rows(0)("ServiceType")
                Else
                    Return New RIAResult With {.ErrorCode = -162, .ErrorMessage = String.Format(Lang.CitemNoMatch, CitemCode)}
                End If
            End Using
                    ...
                    If dtPROther.Rows.Count > 0 Then
                        For Each drPROther As DataRow In dtPROther.Rows
                            ...
                            For Each drPrChangeFaci As DataRow In dtPrChangeFaci.Rows
                                If drPrChangeFaci("SEQNO") = strSEQNO AndAlso String.Format(",{0},", drPrChangeFaci("Delete003Citem").ToString).Contains(String.Format(",{0},", CitemCode)) Then
            ...
            Using SOUtil As New CableSoft.SO.BLL.Utility.Utility(LoginInfo, DAO)
                Using PRCode As DataTable = SOUtil.GetCode(BLL.Utility.CodeType.PRCode, intPRCode.ToString, False)
                    ...
                    Using Customer As DataTable = DAO.ExecQry(_DAL.GetCustomerData(), New Object() {InRow.Item("CustId"), ServiceType})
                        Return Vali.CheckCanPR(PRCode.Rows(0).Item("CodeNo"),
                                               CableSoft.BLL.Utility.Utility.ConvertDBNullToInteger(PRCode.Rows(0).Item("RefNo")),
                                               CableSoft.BLL.Utility.Utility.ConvertDBNullToInteger(PRCode.Rows(0).Item("Interdepend")),
                                               Customer.Rows(0).Item("CustStatusCode"),
                                               Customer.Rows(0).Item("WipCode3"), InRow.Item("CustId"),
                                               ServiceType, Customer.Rows(0).Item("InstAddrNo"), Nothing)
                    End Using
                End Using
            End Using
        End Using
        ...
        Using PR As New PR(LoginInfo, DAO)
            ...
            Using SOUtil As New CableSoft.SO.BLL.Utility.Utility(LoginInfo, DAO)
                Using dtPRCode As DataTable = SOUtil.GetCode(BLL.Utility.CodeType.PRCode, PRCode.ToString(), False)
                    ...
                    End If
                    Using RetWip As DataSet = PR.GetNormalCalculateData(InDataRow.Item("CustId"),
                                                                        ServiceType, PRCode, InDataRow.Item("ResvTime"),
                                                                        SNo, True, WipData)
                            ...
                            Using bll As New CableSoft.SO.BLL.Wip.Utility.Utility(Me.LoginInfo, Me.DAO)
                                ...
                                If dtDataTable.Rows.Count > 0 Then
                                    ...
                                    If FacilityData.Rows.Count > 0 Then
                                        ...
                                        If Not bll.GetChangeFacility(dtDataTable.Rows(0)(1), WipData.Tables("Wip").Rows(0),
                                                                     Nothing, FacilityData.Rows(0), CanChooseCitemCode,
                                                                     WipData.Tables("ChangeFacility")) Then
                                            Return New RIAResult With {.ResultBoolean = False, .ErrorCode = -900,
                                                                        .ErrorMessage = Lang.GetChangeFacilityErr}
                                        End If
                                    End If
                                End If
                            End Using
...
' Values
'   ('256', '新增拆機工單（不進結清）', '使用時機：新增拆機或暫停頻道工單資料於營運系統。
'使用限制：不進行費用結清作業，直接產生停拆機工單。', 'CableSoft.SO.BLL.Wip.PR.dll', 'CableSoft.SO.BLL.Wip.PR.BillingAPI256');
...
'    DATATABLENAME, ERRORCODE, ORD)
' Values
'   ('256', 'ReasonCode', '停拆機原因代碼', 0, 1, 
''SNo', -1, 6);
'Insert into SO1114B
'   (APIID, FIELDNAME, HEADNAME, DATATYPE, MUSTBE, 
'    DATATABLENAME, ERRORCODE, ORD)
' Values
'   ('256', 'ResvTime', '預約時間', 3, 1, 
''SNo', -1, 7);
'Insert into SO1114B
'   (APIID, FIELDNAME, HEADNAME, DATATYPE, MUSTBE, 
'    DATATABLENAME, ERRORCODE, ORD)
' Values
'   ('256', 'AcceptEn', '受理人員代號', 0, 1, 
''SNo', -1, 8);
'Insert into SO1114B
'   (APIID, FIELDNAME, HEADNAME, DATATYPE, MUSTBE, 
'    DATATABLENAME, ERRORCODE, ORD)
...
'    DATATABLENAME, ERRORCODE, ORD)
' Values
'   ('256', 'ReasonDescCode', '停拆機原因細項', 0, 0, 
''SNo', -1, 10);
'Insert into SO1114B
'   (APIID, FIELDNAME, HEADNAME, DATATYPE, MUSTBE, 
'    DATATABLENAME, ERRORCODE, ORD)
' Values
'   ('256', 'CallSeqNo', '互動單號', 0, 0, 
''SNo', -1, 11);
'Insert into SO1114B
'   (APIID, FIELDNAME, HEADNAME, DATATYPE, MUSTBE, 
'    DATATABLENAME, ERRORCODE, ORD)
' Values
'   ('256', 'FaciSeqNo', '設備流水號', 0, 1, 
''SNo', -1, 12);
'Insert into SO1114B
'   (APIID, FIELDNAME, HEADNAME, DATATYPE, MUSTBE, 
'    DATATABLENAME, ERRORCODE, ORD)
' Values
'   ('256', 'CitemCode', '收費項目', 0, 1, 
''SNo', -1, 13);
...
'   From CD019
'   Where CodeNO = '[CitemCode]' And CodeNo Is Null and (ServiceType='I' Or ServiceType='D')
...
'  "ReasonDescCode": "",
'  "CallSeqNo": "",
'  "Faciseqno": "201308230225413,201308230225414",
'  "CitemCode": "2020,2284,2000,2010" }]
'}

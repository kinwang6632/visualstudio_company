Public Class BillingAPI256DAL
    ...
    End Function
    Friend Function GetWorkCodeForCitemCode(ByVal CitemCode As String, ByVal StartNewProd As Integer) As String
        Dim strSQL As String
        strSQL = String.Format("Select CodeNo,Description,ServiceType From CD007 Where REFNO=15 and nvl(Stopflag,0)=0 and Instr(','||CitemCodeStr||',',',{0},')>0", CitemCode)
        If StartNewProd = 1 Then            
            strSQL = String.Format("Select  CodeNo,Description,ServiceType From CD007 Where REFNO = 15 AND CodeNo In (" & _
                    " Select WorkCode From CD116E Where WorkerType='P' " & _
                        "And ProductCode IN (Select ProductCode From CD019 Where CODENO IN ({0})   ) )", CitemCode)
        End If
        'API 測試報告 ,不能用CD007 要用CD116E關聯 By Kin 2019/11/18 For debby
